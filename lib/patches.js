...
var findPosition = require('./context').findPosition;
...
function applyTest(x, test, options) {
	...
	if(Array.isArray(target)) {
		index = findIndex(options.equals, target, parseArrayIndex(pointer.key), test.context);
		value = target[index];
	} else {
...
function applyAdd(x, change, options) {
	...
	_add(pointer, val, change.context, options.equals);
	return x;
}

function _add(pointer, value, context, equals) {
	var target = pointer.target;
	var index;
	...
	if(Array.isArray(target)) {
		...
		if(pointer.key === '-') {
			...
			index = findIndex(equals, target, parseArrayIndex(pointer.key), context);
...
function invertAdd(pr, add) {
	...
	if(context !== void 0) {
		context = {
			...
			after: cons(add.value, context.after)
		}
	}
...
function applyReplace(x, change, options) {
	...
	if(Array.isArray(target)) {
		index = findIndex(options.equals, target, parseArrayIndex(pointer.key), change.context);
		target[index] = value;
	} else {
		target[pointer.key] = value;
	}
...
function invertReplace(pr, c, i, patch) {
	...
	if(context !== void 0) {
		context = {
			before: context.before,
			after: cons(prev.value, tail(context.after))
		}
	}
...
function applyRemove(x, change, options) {
	...
	_remove(pointer, change.context, options.equals);
	return x;
}

function _remove (pointer, context, equals) {
	var target = pointer.target;
	var index;
...
function applyMove(x, change, options) {
	...
	_add(pto, _remove(pfrom, change.fromContext, options.equals), change.context, options.equals);
	return x;
}
...
function applyCopy(x, change, options) {
	...
	if(Array.isArray(target)) {
		index = findIndex(options.equals, target, parseArrayIndex(pfrom.key), change.fromContext);
		value = target[index];
	} else {
		value = target[pfrom.key];
	}

	_add(pto, clone(value), change.context, options.equals);
	return x;
}
...
function findIndex(equals, a, start, context) {
	var index = start;
	...
	if(context !== void 0 && typeof equals === 'function') {
		index = findPosition(equals, a, context, start);
		if(index < 0) {
			throw new InvalidPatchOperationError('could not find patch context ' + context);
		}
	}
