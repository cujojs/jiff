...
function commuteReplace(replace, b) {
	if(b.op === 'test') {
		return commuteTest(b, replace);
	}

	if(replace.path === b.path && b.op === 'remove') {
		return [{ op: b.op, path: b.path }, { op: 'noop' }];
	}
...
function commuteMove(move, b) {
	if(move.path === b.path && b.op === 'remove') {
		return [{ op: 'remove', path: move.from }, { op: 'noop' }]
	}
 ...
 */
function applyReplace(x, change) {
	var pointer = find(x, change.path);
